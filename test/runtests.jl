...
function test_solve(::Type{T}) where {T}
  for n ∈ 1:(T === Float32 ? 100 : 200)
    ...
    for m ∈ max(1, n - 10):n+10
      ...
      @test TriangularSolve.rdiv!(res, A, UnitUpperTriangular(B)) * UnitUpperTriangular(B) ≈
            A
      ...
      @test UnitLowerTriangular(B) * TriangularSolve.ldiv!(res, UnitLowerTriangular(B), A) ≈
            A
      @test LowerTriangular(B) *
            TriangularSolve.ldiv!(res, LowerTriangular(B), A, Val(false)) ≈ A
      @test UnitLowerTriangular(B) *
            TriangularSolve.ldiv!(res, UnitLowerTriangular(B), A, Val(false)) ≈ A
    end
  end
end
