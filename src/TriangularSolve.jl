...
using LoopVectorization, VectorizationBase, LinearAlgebra
using VectorizationBase: vfnmadd_fast, AbstractStridedPointer, AbstractMask

@generated function solve_AU(A::VecUnroll{Nm1},U::AbstractMatrix) where {Nm1}
  N = Nm1 + 1
  quote
    $(Expr(:meta,:inline))
...
@generated function rdiv_solve_W_u3!(spc, spa, U, lbm, n, ubn, ::StaticInt{W}) where {W}
  quote
    $(Expr(:meta,:inline))
    lbn = n + 1
    Nrange = lbn:ubn
    ...
    for nk ∈ 1:n # nmuladd
      ...
      Base.Cartesian.@nexprs $W c -> C11_c = vfnmadd_fast(A11, @inbounds(U[nk,n+c]), C11_c)
    end
    C11vu = solve_AU(VecUnroll((Base.Cartesian.@ntuple $W C11)), view(U, Nrange, Nrange))
    vstore!(spc, C11vu, Unroll{2,1,$W,1,$W,0x0000000000000000,1}(Unroll{1,$W,3,1,$W,0x0000000000000000,1}((lbm,lbn))))
  end
end
@generated function rdiv_solve_W_u1!(spc, spa, U, lbm, n, ubn, mask::AbstractMask{W}) where {W}
  quote
    $(Expr(:meta,:inline))
    lbn = n + 1
    Nrange = lbn:ubn
    ...
    C11 = solve_AU(C11, view(U, Nrange, Nrange))
...
function rdiv_U!(C::AbstractMatrix{T}, A::AbstractMatrix{T}, U::AbstractMatrix{T}) where {T}
  ...
  for _ ∈ 1:Md
    ...
    for i ∈ 1:Nrd8
      ubn = n+W
      rdiv_solve_W_u3!(spc, spa, U, lbm, n, ubn, WS)
      n = ubn
    end
    for _ ∈ 1:Nd
      lbn = n+1
  ...
  for _ ∈ 1:Mrd8+(Mrr8>0)
    ...
    for i ∈ 1:Nrd8
      ubn = n+W
      rdiv_solve_W_u1!(spc, spa, U, lbm, n, ubn, mask)
      n = ubn
    end
    for _ ∈ 1:Nd
      lbn = n+1
